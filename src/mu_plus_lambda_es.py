...
def mu_plus_lambda_es(returns, generations, mu, lambd, mutation_rate):
    population = initialize_population(mu, len(returns))
    for _ in range(generations):
        # Generate offspring population by mutating individuals from the parent population
        offspring = [mutate(random.choice(population), mutation_rate) for _ in range(lambd)]
        
        # Combine parent and offspring populations
        combined_population = population + offspring
        
        # Sort the combined population by fitness and select the top μ individuals
...
# Run (μ + λ) ES
best_portfolio_mu_plus_lambda = mu_plus_lambda_es(expected_returns, generations, mu, lambd, initial_mutation_rate)
print("Best Portfolio Weights (μ + λ ES):", best_portfolio_mu_plus_lambda)
print("Expected Return (μ + λ ES):", fitness(best_portfolio_mu_plus_lambda, expected_returns))
